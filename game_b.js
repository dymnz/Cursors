...
function onGameOver()
{
	...
	for(var i = 0;i < chosenPlayer.length;i++){
		chosenPlayer[i].getSocket().emit("congrats");
		broadcastAllConsoles("success", {server:"b",teamId:chosenPlayer[i].getTeamId(),name: chosenPlayer[i].name});
	}
}
...
function dump(){
	for(var j = 0;j < players[0].length;j++){
		for(var index = 0;index < players[0][j].length;index++){
			players[0][j][index].getSocket().emit("player info",{server:"b",name:players[0][j][index].name});
		}
	}
}
