...
function onGoal(){
	...
	//if has reached the last map
		...
		// add "successful"
		//this.emit("map change", {map: mapIndex});
		broadcastAllConsoles("success", {server:"r",teamId:onGoalPlayer.getTeamId(),name: onGoalPlayer.name});
...
function dump(){
	for(var j = 0;j < players[0].length;j++){
		for(var index = 0;index < players[0][j].length;index++){
			players[0][j][index].getSocket().emit("player info",{server:"r",name:players[0][j][index].name});
		}
	}
}
